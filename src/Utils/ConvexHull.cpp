...
void generateconvexHull(vector<pair<double,double>> coordinates, vector <pair<double,double>>& resHull){
	...
	*/
	...
	/*
		...
		Since the array is already sorted based on polar angles the farthest point is located at the end.
	*/
	...
	for(unsigned i = 3; i < coordinates.size(); ++i){
		/*
			Check the orientation i.e. angle formed from the point at the top of the stack with the current point.
			If its to the right i.e. cw(CONCAVE), it violates convex hull rule
		*/
